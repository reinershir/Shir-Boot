...
public class ChatGPTServiceImpl  implements io.github.reinershir.ai.service.ChatGPTService {
    ...
    @Autowired
    public ChatGPTServiceImpl(MessageHepler messageHepler,DictionaryService dictionaryService,@Value("${spring.profiles.active}") String profiles) {
    ...
    public String chat(String prompt,String openId,String model){
        ...
        //缓存上下文
        messageHepler.cacheContext(sb.toString(), prompt, openId);
	...
	public void completions(String question, String model) {
		log.info("以stream发送prompt：{}",question);
	...
	@Override
	public void chatCompletions(WebRequestMessage request, String openId, String model,SseEmitter sseEmitter) {
		List<Message> allMessage = messageHepler.getCacheContextByOpenId(openId,model);
        //最新GPT-3.5-Turbo模型
        Message message = Message.builder().role(Message.Role.USER).content(request.getPrompt()).build();
        allMessage.add(message);
        ...
        EventSourceListener eventSourceListener = new OpenAISSEEventSourceListener(sseEmitter);
        ChatCompletion chatCompletion = ChatCompletion.builder().model(StringUtils.hasText(model)?model:this.model)
        		.messages(allMessage).build();
        streamClient.streamChatCompletion(chatCompletion, eventSourceListener);
	}
