...
public class IdUtils {
    ...
    private static String key = "REDIS_KEY_SEQ_DEVICE";
    static {
    ...
    /**
     * @Title: getNextSeqByRedis
     * @Description:   生成6位开始自增的ID，使用redis存储,从200000开始
     * @author ReinerShir
     * @date 2021年3月10日
     * @param redisTemplate
     * @return
     */
    public static Long getNextSeqByRedis(StringRedisTemplate redisTemplate) {
    	if(!redisTemplate.hasKey(key)) {
    		redisTemplate.opsForValue().set(key, "200000");
    		return 200000L;
    	}else {
    		Long id = Long.parseLong(redisTemplate.opsForValue().get(key))+1;
    		redisTemplate.opsForValue().set(key, String.valueOf(id));
    		return id;
    	}
    }
